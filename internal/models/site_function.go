// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// SiteFunction site function
//
// swagger:model siteFunction
type SiteFunction struct {

	// branch
	Branch string `json:"branch,omitempty"`

	// created at
	CreatedAt string `json:"created_at,omitempty"`

	// functions
	Functions []interface{} `json:"functions"`

	// id
	ID string `json:"id,omitempty"`

	// log type
	LogType string `json:"log_type,omitempty"`

	// provider
	Provider string `json:"provider,omitempty"`
}

// Validate validates this site function
func (m *SiteFunction) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this site function based on context it is used
func (m *SiteFunction) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *SiteFunction) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *SiteFunction) UnmarshalBinary(b []byte) error {
	var res SiteFunction
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
