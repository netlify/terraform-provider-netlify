// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// BuildSetup build setup
//
// swagger:model buildSetup
type BuildSetup struct {

	// clear cache
	ClearCache bool `json:"clear_cache,omitempty"`

	// image
	Image string `json:"image,omitempty"`
}

// Validate validates this build setup
func (m *BuildSetup) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this build setup based on context it is used
func (m *BuildSetup) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *BuildSetup) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *BuildSetup) UnmarshalBinary(b []byte) error {
	var res BuildSetup
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
