// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// PluginParams plugin params
//
// swagger:model pluginParams
type PluginParams struct {

	// pinned version
	PinnedVersion string `json:"pinned_version,omitempty"`
}

// Validate validates this plugin params
func (m *PluginParams) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this plugin params based on context it is used
func (m *PluginParams) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *PluginParams) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PluginParams) UnmarshalBinary(b []byte) error {
	var res PluginParams
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
